...
class AppStore extends Component {
  state = {whatisClicked: tabsList[0].tabId, searchInput: ''}
  ...
  render() {
    const {whatisClicked, searchInput} = this.state
    const tabFilterRes = appsList.filter(app => app.category === whatisClicked)
    const searchRes = tabFilterRes.filter(item =>
      item.appName.toLowerCase().includes(searchInput),
    )
        ...
        <ul className="tab-container">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isActive={whatisClicked === eachTab.tabId}
            />
          ))}
        </ul>
        <ul className="app-container">
          {searchRes.map(eachApp => (
